...
<div class="w-full h-full max-h-[60vh] max-w-sm flex-shrink-0 bg-white shadow-md rounded-lg overflow-hidden border border-primary p-4">
    ...
    <div class="w-full h-60 flex justify-center items-center p-2">
        <img class="w-full h-full object-cover rounded-md" src="{{ asset('storage/' . $training->banner) }}" alt="{{ $training->title }}">
    </div>

    <!-- Description -->
    <div class="p-2 space-y-2 md:space-y-4 px-4">
        <!-- Title -->
        <h1 class="text-xl text-primary font-bold text-start px-1 py-2">{{ $training->title }}</h1>

        <!-- Stats -->
        <div class="flex flex-wrap justify-start gap-2 text-[.50rem] md:text-xs text-gray-600 p-2">
            <ul>
                <li>
